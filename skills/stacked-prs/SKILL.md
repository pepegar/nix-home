...
4. Create the worktree and branch from the stack top:
   ```bash
   REPO_ROOT=$(git rev-parse --show-toplevel)
   # Walk up to the main worktree root if we're inside a worktree
   MAIN_ROOT=$(git worktree list --porcelain | head -1 | sed 's/^worktree //')
   mkdir -p "$MAIN_ROOT/.worktrees"
   git worktree add -b "$NEW_BRANCH" "$MAIN_ROOT/.worktrees/$NEW_BRANCH" "$TOP_BRANCH"
   ```
...
6. Ask for a branch description and set it:
   ```bash
   git config "branch.$NEW_BRANCH.description" "$DESCRIPTION"
   ```

7. Report: new branch, worktree path, position in stack, parent branch.
...
4. Ensure worktree exists, create if needed:
   ```bash
   MAIN_ROOT=$(git worktree list --porcelain | head -1 | sed 's/^worktree //')
   WORKTREE_PATH="$MAIN_ROOT/.worktrees/$TARGET_BRANCH"
   if [ ! -d "$WORKTREE_PATH" ]; then
     git worktree add "$WORKTREE_PATH" "$TARGET_BRANCH"
   fi
   ```
5. If `$ZELLIJ` is set, rename tab to JIRA issue ID:
   ```bash
   issue_id=$(echo "$TARGET_BRANCH" | grep -oE '^[A-Z]+-[0-9]+' | head -1)
   [ -n "$issue_id" ] && zellij action rename-tab "$issue_id"
   ```
6. Tell the user to `cd $WORKTREE_PATH`.
